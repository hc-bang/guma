...
6. **큐스코 큐니(CUEUNY) 당구 리플레이 아카이브 & 다차원 검색기 (`/cueuny`)**
   ...
   * **원스톱 상세 필터 팝업 모달:**
     * **심플 & 미니멀 화면:** 평소에는 상시 필터 바를 숨겨 경기 영상 카드 목록을 시원하게 바로 감상.
     * **통합 필터 다이얼로그:** [상세필터] 버튼 클릭 시 팝업창에서 검색어, 연도, 월, 승/패 결과 및 에버리지/하이런/경기시간/이닝 수를 한 번에 세팅하고 [필터 적용] 또는 [초기화].
     * **스마트 클라이언트 엔진:** 브라우저 메모리 캐시(`cachedMasterReplays`)를 통해 0.001초 만에 즉각 필터링 및 활성 조건 태그 칩 제공.
